...
**摘要：** 人工智能（AI）算力负荷既可降档运行以减少单位工作量能耗，也可在时间上转移，二者的系统价值来源不同。本文在同等工作量与可靠性下，将实测图形处理器（GPU）功率档位、作业轨迹、投资—运行联合规划模型与企业行为纳入同一框架，以甘肃、江苏、贵州 2030 年公开数据情景分析分时电价形状、事件型合同与按系统边际成本定价对时间转移价值兑现的影响。孤岛情景下系统协调相对刚性运行可降低算力负荷增量系统成本 {f1(isl.red_S0_S2_pct.min())}%～{f1(isl.red_S0_S2_pct.max())}%，高效档位不作转移即可获得绝大部分；时间转移剩余价值仅为增量成本的 {f1(isl.gap_S0e_S2_pct.min())}%～{f1(isl.gap_S0e_S2_pct.max())}%，省间交换时低于 {f1(exc.gap_S0e_S2_pct.max()+0.05)}%。按现行分时电价优化时，18 组孤岛设定中 {neg1} 组的时间转移为负价值；叠加事件型合同不能改善且暴露于基线操纵；按系统边际成本形状定价在除 {negr} 组外的全部设定回收转移价值，与协调最优差距不超过 {f1(rt_isl)}%。近期应优先释放档位价值，时间转移需要形状正确的价格而非叠加合同。

**关键词：** 算力负荷；数据中心；需求响应；分时电价；负荷灵活性；电力系统规划；机制设计

**Shape of Time-of-Use Tariffs and the Value of Temporal Shifting of AI Computing Load: A Scenario Analysis for Chinese Provincial Power Systems**
...
**KEY WORDS:** AI computing load; data centre; demand response; time-of-use tariff; load flexibility; power system planning; mechanism design
...
[1] 国家发展改革委高技术司. “东数西算”全面启动 八枢纽激发数据新活力[EB/OL]. (2022-03-21)[2026-09-17]. https://www.ndrc.gov.cn/fzggw/jgsj/gjss/sjdt/202203/t20220321_1319862.html.
    National Development and Reform Commission, High-Technology Department. "Eastern Data, Western Computing" fully launched: eight hubs energise data[EB/OL]. (2022-03-21)[2026-09-17]. https://www.ndrc.gov.cn/fzggw/jgsj/gjss/sjdt/202203/t20220321_1319862.html (in Chinese).
...
[18] 国家统计局. 中国统计年鉴 2021: 表 9-14 分地区用电量[M]. 北京: 中国统计出版社, 2021.
    National Bureau of Statistics of China. China Statistical Yearbook 2021: Table 9-14 electricity consumption by region[M]. Beijing: China Statistics Press, 2021 (in Chinese).

[19] Potsdam Institute for Climate Impact Research. Data bundle PyPSA-China-PIK: rasters and basic cutout, v1.1 (containing the Global Energy Monitor Global Integrated Power Tracker, July 2025, China subset)[DS/OL]. Zenodo, 2025[2026-09-17]. https://doi.org/10.5281/zenodo.16810831.
