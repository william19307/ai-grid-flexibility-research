...
out={}
for tag,f in [('coal_min_0',T/'regional_smoke_grid_summary.csv'),('coal_min_0.4',T/'regional_smoke_grid_summary_coalmin40.csv')]:
    if not f.exists():continue
    d=pd.read_csv(f);d=d[d.W==24];base=d[d.case=='NOAI'].set_index(['province','export','ai_share']);rows=[]
    for (p,e,a),g in d.groupby(['province','export','ai_share']):
        b=base.loc[(p,e,a)];r={c:g[g.case==c].iloc[0] for c in ['S0','S0b','S1','S2']}
