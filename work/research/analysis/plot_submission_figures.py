...
P=['Gansu','Jiangsu','Guizhou'];C={'S0':'#6e6e6e','S1':'#1f77b4','S3':'#d62728','S1rt':'#ff7f0e','S2':'#2ca02c'};L={'S0':'S0 rigid','S1':'S1 firm, tariff shape','S3':'S3 event contract','S1rt':'S1rt firm, marginal-cost-shaped price','S2':'S2 system-coordinated'}
...
for j,p in enumerate(P):
    ...
    lo=s[[f'cost_{c}' for c in C]].min().min();hi=s[[f'cost_{c}' for c in C]].max().max();ax[j].set_ylim(max(0,lo*0.9),hi*1.03)
ax[0].set_ylabel('Incremental system cost\nper AI MWh (EUR MWh$^{-1}$)');ax[0].legend(fontsize=5.5,frameon=False,loc='lower left')
...
# Fig 4: robustness
mw=pd.read_csv(T/'regional_2030_multiweather.csv');mw=mw[mw.gap_S0_S2_pct.notna()];IDLE=0.41;mc=pd.read_csv(T/'regional_2030_cost_montecarlo.csv');mc=mc[mc.gap_S0_S2_pct.notna()]
fig,ax=plt.subplots(1,3,figsize=(7.2,2.4))
for i,(df,title,col) in enumerate([(mw[mw.idle==IDLE],'a  Ten weather years (2015–2024)','gap_S0_S2_pct'),(mc,'b  Cost-parameter Monte Carlo','gap_S0_S2_pct'),(mw[mw.idle==IDLE],'c  S1rt gap to optimum','gap_S1rt_S2_pct')]):
    ...
    ax[i].set_title(title,fontsize=8.5,loc='left');ax[i].grid(axis='y',alpha=.25,linewidth=.5)
ax[0].set_ylabel('Rigid-to-coordinated gap (%)');ax[1].set_ylabel('Rigid-to-coordinated gap (%)');ax[2].set_ylabel('Gap to coordinated optimum (%)')
