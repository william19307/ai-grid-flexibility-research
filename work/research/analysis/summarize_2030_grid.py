...
d=pd.read_csv(T/'regional_2030_grid_summary.csv');d=d[d.total_cost.notna()]
keys=['province','export','ai_share','slack_mult','slack_base_h'];base=d[d.case=='NOAI'].set_index(keys);rows=[]
for k,g in d.groupby(keys):
    b=base.loc[k];r={c:g[g.case==c].iloc[0] for c in ['S0','S1','S2','S3'] if (g.case==c).any()}
    inc=lambda c:(r[c].total_cost-b.total_cost)/r[c].ai_mwh;co2=lambda c:(r[c].emissions_t-b.emissions_t)/r[c].ai_mwh
    rows.append(dict(zip(keys,k))|dict(cost_S0=inc('S0'),cost_S1=inc('S1'),cost_S2=inc('S2'),cost_S3=inc('S3'),gap_S0_S2_pct=(inc('S0')/inc('S2')-1)*100,gap_S1_S2_pct=(inc('S1')/inc('S2')-1)*100,gap_S3_S2_pct=(inc('S3')/inc('S2')-1)*100,
        co2_S0=co2('S0'),co2_S1=co2('S1'),co2_S2=co2('S2'),co2_S3=co2('S3'),curtail_NOAI=b.curtail,curtail_S2=r['S2'].curtail,
...
x=pd.DataFrame(rows).round(3);x.to_csv(T/'regional_2030_gaps.csv',index=False)
pd.set_option('display.width',300);pd.set_option('display.max_columns',40)
print(x[['province','export','ai_share','slack_mult','cost_S0','cost_S1','cost_S2','cost_S3','gap_S0_S2_pct','gap_S1_S2_pct','gap_S3_S2_pct','curtail_NOAI','new_ocgt_S0','new_ocgt_S2','new_batt_S0','new_batt_S2','s3_system_saving_vs_S1','s3_compensation_floor','s3_mean_commitment_mw']].to_string(index=False))
# figure: cost per AI MWh by case, W-slack (1,6), three provinces, export on/off, ai 10%
fig,axes=plt.subplots(2,3,figsize=(12,6.2),sharey='row');cases=['S0','S1','S2','S3'];cols=['#7f7f7f','#1f77b4','#2ca02c','#d62728'];lab={'S0':'S0 rigid','S1':'S1 firm-autonomous','S2':'S2 system-coordinated','S3':'S3 commitment mechanism'}
for j,prov in enumerate(['Gansu','Jiangsu','Guizhou']):
    ...
    for i,(metric,yl) in enumerate([('cost','Incremental system cost per AI MWh (EUR/MWh)'),('co2','Incremental emissions per AI MWh (tCO2/MWh)')]):
        ax=axes[i,j];groups=[(e,a) for e in [True,False] for a in [0.05,0.10,0.20]];xt=np.arange(len(groups));w=0.2
        for k_,c in enumerate(cases):
            vals=[sub[(sub.export==e)&(sub.ai_share==a)][f'{metric}_{c}'].values[0] for e,a in groups]
            ax.bar(xt+(k_-1.5)*w,vals,w,color=cols[k_],label=lab[c] if (i==0 and j==0) else None,linewidth=0)
...
fig.suptitle('2030 provincial scenario on public data (GEM 2030 fleet, archive 2030 load ratio and costs, official TOU shapes, Helios-derived batches). Hourly load shape unvalidated; external-market proxy; documented assumptions.',fontsize=8,color='dimgray')
fig.tight_layout(rect=[0,0,1,0.95])
for ext in ['png','svg','pdf']:fig.savefig(F/f'regional_2030_cost_emissions.{ext}',dpi=160)
print('figure saved')
